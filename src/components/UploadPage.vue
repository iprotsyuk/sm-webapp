   ...
   methods: {
     onUpload(filenames) {
      const targetFileExt = 'mzml';
       ...
       const imzml = filenames.filter(f => f.toLowerCase().endsWith(targetFileExt))[0];
       Vue.nextTick(() => {
         this.$refs.editor.suggestDatasetName(imzml.slice(0, imzml.length - targetFileExt.length));
       });
     },
